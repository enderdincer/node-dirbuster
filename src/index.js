...
function buster(options) {
  ...
  options.methods.forEach(function (method) {
    switch (method) {
      case "HEAD":
        attachCollector("HEAD");
        break;
      case "GET":
        attachCollector("GET");
        break;
      case "POST":
        attachCollector("POST");
        break;
      case "PUT":
        attachCollector("PUT");
        break;
      case "DELETE":
        attachCollector("DELETE");
        break;
    }
  });

  function attachCollector(method) {
    var collectorDel = createCollectorStream(
      options.url,
      method,
      tfactor,
      options.extension
  ...
  switch (options.export) {
    ...
    case "csv":
      break;
    default:
      exportStream = exportStreams.createToJSON();
      break;
  }

  collectorsFunil.pipe(exportStream).pipe(options.outStream);
